...
from model import *
import numpy as np
from data_loader import Scaling, ImageFolder, random_downscale
...
class Solver(object):
    ...
    def build_model(self, config):
        ...
        self.discriminator = Discriminator(input_shape=(3, *self.hr_shape)) # 3 channels  - RGB 
    ...
    def img_add_info(self, img_paths, img, step, loss):
        ...
        d.text((10,10), "model trained for {} steps, loss G: {:.4f}".format(step, loss.item()) + \
               "\n" + '\n'.join([os.path.basename(path) for path in img_paths]), fill='black')
        imgs_comb = np.hstack((np.array(img), added_text))
        
        d.text((10,10), "model trained for {} steps, loss G: : {:.4f}".format(step, loss.item()), fill='black')
...
#         Tensor = torch.cuda.FloatTensor if cuda else torch.Tensor    
        ...
        for step in range(self.start_step, self.total_step):
            ...
            img_paths, lr_images, hr_images = next(data_iter)
            ...
            if self.content_loss_function !=None:
                ...
                # Total loss loss_G = 1e-3*loss_GAN + loss_content
            ...
            loss_real = criterion_GAN(self.discriminator(hr_images), valid)
            ...
            if (step+1) % self.model_save_step == 0:                
                self.save('generator', step+1, loss_G.item(), os.path.join(self.model_save_path, '{}.pth.tar'.format(str(step+1) + '_cont_'+str(self.content_loss_function).upper() + '_GEN_' + '_loss_gen_' + "%.3f" % loss_G.item() + '_loss_disc_'+ "%.3f" % loss_D.item())))
                self.save('discriminator', step+1, loss_D.item(), os.path.join(self.model_save_path, '{}.pth.tar'.format(str(step+1) + '_cont_'+str(self.content_loss_function).upper() + '_DISC_'+ '_loss_gen_' + "%.3f" % loss_G.item() + '_loss_disc_'+ "%.3f" % loss_D.item())))
...
#       lr_image_with_kernel = self.randkern.ConcatDegraInfo(lr_image_scaled)
        ...
        lr_image= lr_image_scaled
        lr_image= lr_image.to(self.device)

        reconst = self.model(lr_image)
        ...
        #Create Grid
        hr_image_hat_np_scaled = hr_image_hat_np
